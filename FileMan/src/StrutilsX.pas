...
function IsEmptyStr(const S: string; EmptyChars: TCharSet): Boolean;
...
{ NPos searches for a N-th position of character C in a given string. }
...
function AnsiProperCase(const S: string; WordDelims: TCharSet): string;
...
function ExtractDelimited(N: Integer; const S: string; Delims: TCharSet): string;
{ ExtractWord, ExtractWordPos and ExtractDelimited given a set of word
  delimiters, return the N'th word in S. }

function ExtractSubstr(const S: string; var Pos: Integer; Delims: TCharSet): string;
{ ExtractSubstr given a set of word delimiters, return the substring from S,
  that started from position Pos. }

function IsWordPresent(const W, S: string; WordDelims: TCharSet): Boolean;
...
{$ELSE}
Uses SysUtils, WinTypes, WinProcs, Str16;
{$ENDIF}
...
begin
  ...
  if Length(Result) > 0 then
{$IFDEF WIN32}
    OemToAnsi(PChar(OemStr), PChar(Result));
{$ELSE}
  begin
    StrPCopy(Src, OemStr);
    OemToAnsi(Src, Dest);
